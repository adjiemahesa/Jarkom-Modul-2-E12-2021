...
' > /etc/apache2/sites-available/general.mecha.franky.E12.com-15000.conf

cd /etc/apache2/sites-available
cp 000-default.conf general.mecha.franky.E12.com-15500.conf
cd

echo 'VirtualHost *:15500>
        # The ServerName directive sets the request scheme, hostname and port t$
        # the server uses to identify itself. This is used when creating
        # redirection URLs. In the context of virtual hosts, the ServerName
        # specifies what hostname must appear in the request's Host: header to
...
# vim: syntax=apache ts=4 sw=4 sts=4 sr noet
' > /etc/apache2/sites-available/general.mecha.franky.E12.com-15500.conf
...
echo ' # If you just change the port or add more ports here, you will likely al$
...
cp -r general.mecha.franky /var/www/general.mecha.franky.E12.com
